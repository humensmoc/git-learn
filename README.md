...
**Git Learn** is an interactive web app for learning Git from the command line. Type real Git commands in a built-in terminal, watch the repository state update in real time, and progress through 13 guided lesson worlds — from `git init` to branching, merging, stashing, and rollback.
...
- **13 lesson worlds** — progressive curriculum covering init, remote, clone, status, commit flow, branches, merge/rebase, stash, undo, risky operations, `.gitignore`, collaboration, and a real-engine finale
- **Dual engine architecture**
  - **Sim mode** — lightweight simulated Git engine for fast, safe practice (most lessons)
  - **Real mode** — [isomorphic-git](https://isomorphic-git.org/) runs a real Git repository entirely in the browser (World 13)
...
| # | Topic | Engine |
|---|-------|--------|
| 1 | Initialize a repository | Sim |
| 2 | Connect to GitHub | Sim |
| 3 | Clone & sync | Sim |
| 4 | Status & diagnostics | Sim |
| 5 | Commit workflow | Sim |
| 6 | Branch operations | Sim |
| 7 | Merge & rebase | Sim |
| 8 | Stash | Sim |
| 9 | Undo basics | Sim |
| 10 | Risk & rollback | Sim |
| 11 | `.gitignore` | Sim |
| 12 | Collaboration wrap-up | Sim |
| 13 | Real sandbox finale | Real |
...
**Git Learn** 是一个在浏览器中学习的 Git 命令行教学网站。在内置终端里输入真实的 Git 命令，右侧图谱与文件面板会实时反映仓库变化，并通过 13 个引导式关卡，从 `git init` 一路练到分支、合并、暂存、撤销与协作。
...
- **13 个教学关卡** — 覆盖初始化、远程关联、克隆同步、状态诊断、提交流程、分支、合并/变基、stash、撤销、风险回滚、`.gitignore`、协作收尾，以及真实引擎收官
- **双引擎架构**
  - **Sim 模式** — 轻量模拟引擎，适合快速、安全地练习（大部分关卡）
  - **Real 模式** — 基于 [isomorphic-git](https://isomorphic-git.org/)，在浏览器内运行真实 Git 仓库（第 13 关）
